...
export const createOrderSchema = z.object({
  ...
  if (data.fulfillment === "DELIVERY") {
    if (!data.deliveryZoneId) ctx.addIssue({ code: "custom", path: ["deliveryZoneId"], message: "Selecione a região de entrega" });
    if (!data.address) ctx.addIssue({ code: "custom", path: ["address"], message: "Informe o endereço" });
  }
});
