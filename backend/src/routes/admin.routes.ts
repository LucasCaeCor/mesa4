...
import { verifySecondFactor } from "./pix-security.routes.js";
...
import { sendAdminPushTest } from "../modules/push/admin-push.service.js";
export async function adminRoutes(app: FastifyInstance) {
  app.post(
    ...
    async (request) => {
      ...
      // Depois que o Authenticator é configurado na área PIX,
      // ele também passa a proteger todo novo login administrativo.
      if (admin.pixTotpEnabled) {
        if (!input.code) {
          throw new HttpError(
            401,
            "Informe o código do Authenticator para entrar",
            "ADMIN_TOTP_REQUIRED",
          );
        }
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (
        ...
        throw new HttpError(
          401,
          "Sua senha administrativa atual é inválida",
          "INVALID_CURRENT_ADMIN_PASSWORD",
        );
      }
      ...
      if (existing) {
        throw new HttpError(
          409,
          "Já existe uma conta administrativa com este e-mail",
...
app.get("/admin/me", { preHandler: app.authenticateAdmin }, async (request) => ({ admin: request.user }));
  ...
  app.post(
    ...
    async (request, reply) => {
      const input = z
        .object({
          fid: z.string().trim().min(10).max(500),
          platform: z
            .enum(["ANDROID"])
            .default("ANDROID"),
          deviceName: z
